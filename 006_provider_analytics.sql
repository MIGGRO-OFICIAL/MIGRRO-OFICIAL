-- ============================================================================
-- MIGGRO - Dashboard do Prestador - Estrutura de Analytics (VERSÃO IDEMPOTENTE)
-- ============================================================================
-- 
-- INSTRUÇÕES:
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_provider_analytics_provider') THEN
        CREATE INDEX idx_provider_analytics_provider ON provider_analytics(provider_id, date DESC);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_provider_analytics_service') THEN
        CREATE INDEX idx_provider_analytics_service ON provider_analytics(service_id, date DESC);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_provider_analytics_type') THEN
        CREATE INDEX idx_provider_analytics_type ON provider_analytics(metric_type);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_provider_analytics_date') THEN
        CREATE INDEX idx_provider_analytics_date ON provider_analytics(date DESC);
    END IF;
END $$;
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_service_views_service') THEN
        CREATE INDEX idx_service_views_service ON service_views(service_id, viewed_at DESC);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_service_views_viewer') THEN
        CREATE INDEX idx_service_views_viewer ON service_views(viewer_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_service_views_date') THEN
        CREATE INDEX idx_service_views_date ON service_views(viewed_at DESC);
    END IF;
END $$;
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profile_views_profile') THEN
        CREATE INDEX idx_profile_views_profile ON profile_views(profile_id, viewed_at DESC);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profile_views_viewer') THEN
        CREATE INDEX idx_profile_views_viewer ON profile_views(viewer_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profile_views_date') THEN
