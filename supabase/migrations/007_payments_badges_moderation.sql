...
-- Nota: As tabelas 'badges' e 'user_badges' já foram criadas na migration 0001_initial_schema.sql
-- A tabela 'badges' na 0001 tem: id, name, name_pt, description, icon, color, rarity, created_at
-- A tabela 'user_badges' na 0001 tem: id, user_id, badge_id, earned_at
-- Índice já criado na 0001: idx_user_badges_user
-- Não tentamos recriar essas tabelas ou índices duplicados aqui

-- ============================================================================
-- 3. SISTEMA DE MODERAÇÃO
-- ============================================================================

-- Nota: A tabela 'reports' já foi criada na migration 0002_admin_tables.sql
-- A tabela já tem: target_type, target_id (não report_type, reported_id)
-- Índices já criados na 0002: idx_reports_target, idx_reports_status, idx_reports_created
-- Apenas criamos índices adicionais que não existem

CREATE INDEX IF NOT EXISTS idx_reports_reporter ON reports(reporter_id);

-- Nota: A tabela 'moderation_actions' já foi criada na migration 0002_admin_tables.sql
-- Índices já criados na 0002: idx_moderation_actions_target, idx_moderation_actions_moderator, idx_moderation_actions_created
-- Apenas criamos índices adicionais que não existem
...
-- Badges (públicos)
-- Nota: A tabela 'badges' já foi criada na migration 0001 sem a coluna 'is_active'
ALTER TABLE badges ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Anyone can view active badges" ON badges;
-- Política simplificada: todos podem ver todos os badges (já que não há coluna is_active)
